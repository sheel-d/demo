window.onload = function(){
	...
	function fapai(This){//开始游戏发牌
		var index= Math.floor((Math.random()*aArr.length));
		var index1 = Math.floor((Math.random()*aArr.length));		

		var sSpan = cDiv[This].getElementsByTagName('span')[0];
		...
		oImg.className = 'oo';
		oImg.src='img/0.gif';
		cImg[This].appendChild(oImg);

		cImg[This].innerHTML +='<img src='+aArr[index].img+" i="+index+'>';
		cImg[This].innerHTML +='<img src='+aArr[index1].img+" i="+index1+'>';
	...
	function kongzhi(num){//按钮事件控制
		var aInput = cDiv[num].getElementsByTagName('input');
		aInput[0].onclick = function(){ //在要一张牌
			if(bBt == false){
				if(tBt){
					var sSpan = cDiv[num].getElementsByTagName('span')[0];
					var index = Math.floor((Math.random()*aArr.length));
					cImg[num].innerHTML +='<img src='+aArr[index].img+' i='+index+'>';
					...
					if(sSpan.innerHTML>21){
						alert("非常遗憾！，你爆了！");
						bBt = true;
						var eb = cDiv[num].getElementsByTagName('b')[1];
						eb.innerHTML = 2;

						var cText = cDiv[num]. getElementsByTagName('input')[5];
						var eb = cDiv[num].getElementsByTagName('b')[1];
						var ebq = cDiv[num].getElementsByTagName('b')[2];
		...
		aInput[4].onclick = function(){//保险
			if(tBt){
				var Aimg = cImg[num].getElementsByTagName('img');
				var jiaB =cDiv[num].getElementsByTagName('b')[3];
		...
		aInput[7].onclick = function(){//选择A为11，默认是1
			var Aimg = cImg[num].getElementsByTagName('img');
			var iu = Number(Aimg[2].getAttribute('i'));
			var tu = Number(Aimg[1].getAttribute('i'));
			var sSpan = cDiv[num].getElementsByTagName('span')[0];
		...
		//alert(1);
		var yOo = cDiv[num].getElementsByClassName('oo');
		yOo[num].onmouseover = function(){ //鼠标移入，暗牌隐藏
			yOo[num].style.display = "none";
		};
		//alert(2);
		var dImg = cDiv[num].getElementsByTagName('img');
		dImg[1].onmouseout = function(){//鼠标移出，暗牌显示
			yOo[num].style.display = "block";
		};
		//alert(3);
	}
};